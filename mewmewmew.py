...
#OBJECTIVE FUNCTION (FIXED: Standardized time parameters for daily tracking metrics)
#-------------------
...
fg_comp6 = (-9.592126e-03 * P_node[15]) + 
               (-2.268387e-03 * T_node[15]) + 
               (6.537890e-05 * Q_comp[6]) + 
               (3.071554e-04 * s_comp[6]) + 
               (2.241358e-01 * n_comp[6]) - 0.777623;
...
print(f"Total Convergence Time   : {solving_time:.4f} seconds")

print("       OPTIMIZED NETWORK RESULTS        ")
print("="*40)

# 1. Retrieve and print Node Pressures (P_node)
print("Node Pressure Profiles (MPa):")
print("-" * 30)
p_node_var = ampl.get_variable("P_node")
# Fetch the values as a pandas DataFrame or loop directly
for node, value in p_node_var.get_values().to_dict().items():
    print(f"  Node {node:2}: {value:.4f} MPa")

print("\n" + "-"*30 + "\n")

# 2. Retrieve and print Node Temperatures (T_node)
print("Node Temperature Profiles (K):")
print("-" * 30)
t_node_var = ampl.get_variable("T_node")
for node, value in t_node_var.get_values().to_dict().items():
    print(f"  Node {node:2}: {value:.2f} K")

print("\n" + "-"*30 + "\n")

# 3. Retrieve and print Pipeline Volumetric Flow Rates (Q_pipe)
print("Pipeline Segment Flow Rates (10^4 m³/d):")
print("-" * 30)
q_pipe_var = ampl.get_variable("Q_pipe")
for segment, value in q_pipe_var.get_values().to_dict().items():
    print(f"  Segment {segment:2} (Node {segment} -> Node {segment+1}): {value:.4f} 10^4 m³/d")
print("="*40)